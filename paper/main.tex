...
\definecolor{bluepale}{HTML}{C6DBEF}
\definecolor{orange}{HTML}{E6550D}
\definecolor{orangepale}{HTML}{FDBE85}
\definecolor{coral}{HTML}{E34A33}
\definecolor{contamination}{HTML}{FFF3CD}
...
% --- Title ---
\title{\textbf{Epistemic Blinding: An Inference-Time Protocol for Auditing Prior Contamination in LLM-Assisted Analysis}}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
& \textbf{Overlap} & \textbf{Only B} & \textbf{Only U} & \textbf{Tgt} \\
\midrule
AML & 90\% & 2 & 2 & 6/6 \\
...
\bottomrule
\end{tabular}
\caption{Set overlap between blinded and unblinded top-20 lists. Target recovery (rightmost column) is identical in both conditions.}
\label{tab:overlap}
\end{table}
...
The most direct evidence of contamination comes from the model's own justifications (Table~\ref{tab:smokinggun}).

% --- Figure 2: Smoking gun table ---
\begin{table*}[t]
\centering
\small
\begin{tabular}{@{}p{1.8cm}p{6.8cm}p{6.8cm}@{}}
\toprule
& \cellcolor{bluepale}\textcolor{blue}{\textbf{Blinded (Gene\_088, ranked \#5)}} & \cellcolor{orangepale}\textcolor{orange}{\textbf{Unblinded (KRAS, ranked \#1)}} \\
\midrule
\textbf{Opening} & ``Gene\_088 ranks \#5 based on quantitative mutation features'' & ``KRAS ranks \#1 as the most actionable target'' \\
\addlinespace
\textbf{Shared reasoning} & \multicolumn{2}{c}{\textit{Both cite: high mutation frequency (38\%), recurrent codon-12 hotspot, strong statistical enrichment, exclusively missense mutations}} \\
\addlinespace
\textbf{Additional reasoning} & ``GTPase domain concentrated mutations suggest gain-of-function mechanism'' & \cellcolor{contamination}\textcolor{coral}{\textbf{``Proven therapeutic tractability via covalent RAS inhibitors (sotorasib, adagrasib)''}} \newline \cellcolor{contamination}\textcolor{coral}{\textbf{``Central node in RAS/MAPK pathway with extensive clinical validation''}} \\
\addlinespace
\textbf{Conclusion} & ``Exceptionally high recurrence at a single hotspot with strong functional convergence'' & ``KRAS is the defining oncogenic driver in CRC with validated druggability and multiple approved therapies'' \\
\bottomrule
\end{tabular}
\caption{Side-by-side LLM justifications for the same gene (KRAS / Gene\_088) in colorectal cancer. The highlighted phrases in the unblinded column are not derivable from any provided feature---they come from the model's parametric knowledge. Same gene, same features, different rank (\#5 vs.\ \#1), different reasoning.}
\label{tab:smokinggun}
\end{table*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.65\textwidth]{figures/fig3_slope_chart.png}
\caption{Rank-shift slope chart for IDH-wildtype glioblastoma. Lines connect the same gene's rank in the blinded (left) and unblinded (right) conditions. Orange: famous genes promoted when named. Blue: obscure genes with strong features demoted when named.}
\label{fig:slope}
\end{figure*}
...
We ran five independent seeds (42, 137, 256, 389, 501). Results:
